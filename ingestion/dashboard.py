...
def load_data():
    news = pd.read_csv(
        NEWS_CSV, parse_dates=["published_at", "ingested_at"], low_memory=False
    )
    prices = pd.read_csv(
        PRICES_CSV, parse_dates=["timestamp", "ingested_at"], low_memory=False
    )
    return news, prices
...
with col2:
    ...
    date_range_p = st.date_input(
        "Rango de fechas (precios)", [min_date_p, max_date_p], key="pr"
    )

# Filtrar datos
news_f = news[
    (news["symbol"] == symbol)
    & (news["published_at"] >= str(date_range[0]))
    & (news["published_at"] <= str(date_range[1]))
]
prices_f = prices[
    (prices["symbol"] == symbol)
    & (prices["timestamp"] >= str(date_range_p[0]))
    & (prices["timestamp"] <= str(date_range_p[1]))
]
...
if not news_f.empty:
    fig_sent = px.scatter(
        ...
        title="Sentimiento de Noticias a lo largo del tiempo",
    )
...
if not news_f.empty and not prices_f.empty:
    ...
    merged = pd.merge_asof(
        news_f.sort_values("published_at"),
        prices_f.sort_values("timestamp"),
    ...
    if not merged.empty:
        fig_corr = px.scatter(
            ...
            y="close",
            trendline="ols",
            title="Sentimiento vs Precio de Cierre",
        )
