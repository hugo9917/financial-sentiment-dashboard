...
class DashboardTester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "access_token" in data:
                    ...
                    self.log_test(
                        "Authentication Login", True, "Token received", response_time
                    )
    ...
    def test_frontend_accessibility(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self.log_test(
                    "Frontend Accessibility", True, "Frontend is running", response_time
                )
                return True
            else:
                self.log_test(
            ...
            self.log_test(
                "Frontend Accessibility", False, f"Frontend not accessible: {str(e)}", 0
            )
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if summary:
                    ...
                    valid_scores = all(
                        -1 <= item.get("avg_score", 0) <= 1 for item in summary
                    )
                    ...
                    if valid_scores:
                        self.log_test(
                            "Data Quality",
                            True,
                            f"Valid sentiment scores in {len(summary)} categories",
                        )
                        return True
                    else:
                        self.log_test(
                            "Data Quality", False, "Invalid sentiment scores detected"
                        )
                        return False
                else:
                ...
                self.log_test(
                    "Data Quality", False, f"API error: {response.status_code}"
                )
                return False
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:8000", help="Backend API URL"
    )
    parser.add_argument(
