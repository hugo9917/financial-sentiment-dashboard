...
class AdvancedRateLimiter:
    ...
    def _check_user_limit(self, client_ip: str, limit: str) -> bool:
        ...
        if len(self.request_history[client_ip]) >= requests_per_minute:
            logger.warning(f"Rate limit exceeded for IP {client_ip}: {len(self.request_history[client_ip])} requests in last minute")
            return True
    ...
    def get_rate_limit_info(self, client_ip: str) -> Dict:
        ...
        is_blocked = client_ip in self.blocked_ips and time.time() < self.blocked_ips[client_ip]
        ...
        return {
            ...
            "block_until": self.blocked_ips.get(client_ip, None) if is_blocked else None
        }
...
def create_rate_limiting_middleware(limiter: Limiter):
    ...
    async def rate_limiting_middleware(request: Request, call_next):
        ...
        user_role = "free"
        try:
            # Intentar obtener el usuario actual (esto dependerá de tu implementación de auth)
        ...
        response.headers["X-RateLimit-Remaining"] = str(max(0, 60 - rate_limit_info["requests_last_minute"]))
