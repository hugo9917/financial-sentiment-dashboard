...
class RateLimitTester:
    ...
    def test_endpoint(
        ...
        try:
            ...
            return {"status_code": 0, "error": str(e), "body": None}

    def test_rate_limiting(
        self, endpoint: str, max_requests: int = 10, delay: float = 0.1
    ) -> List[Dict]:
        """
        ...
        Returns:
            Lista de respuestas
        """
        print("🧪 Probando rate limiting en: {endpoint}")
        print("📊 Haciendo {max_requests} requests con delay de {delay}s")

        responses = []

        for i in range(max_requests):
            print("  Request {i+1}/{max_requests}...")
    ...
    def test_login_rate_limiting(self) -> List[Dict]:
        ...
        for i in range(10):
            ...
            if response["status_code"] == 200:
                print("    ✅ 200 OK (login exitoso)")
            elif response["status_code"] == 401:
                print("    ❌ 401 Unauthorized (credenciales incorrectas)")
            elif response["status_code"] == 429:
                print("    ⚠️  429 Rate Limited (demasiados intentos)")
            else:
                print("    ❓ {response['status_code']} Otro error")
    ...
    def test_data_endpoints(self) -> Dict[str, List[Dict]]:
        ...
        for endpoint in endpoints:
            ...
            print(
                f"  📈 Resultados: {success_count} exitosos, "
    ...
    def run_full_test(self):
        ...
        for endpoint, responses in data_results.items():
            ...
            print(
                f"📊 {endpoint}: "
                f"{rate_limited} rate limited de {len(responses)} requests"
            )

        print(
            f"\n📈 Total: {total_data_rate_limited} rate limited de "
            f"{total_data_requests} requests"
        )
