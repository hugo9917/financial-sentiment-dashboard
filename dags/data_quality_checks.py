    ...
    for file in files[:3]:  # Validar solo los primeros 3 archivos
        try:
            ...
            if isinstance(data, list):
                validation_results.append(f"{file}: Valid JSON array with {len(data)} items")
            elif isinstance(data, dict):
                validation_results.append(f"{file}: Valid JSON object with {len(data)} keys")
            else:
...
structure_check = PythonOperator(
    ...
    python_callable=validate_data_structure,
    dag=dag
)
