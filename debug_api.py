...
def test_alpha_vantage():
    ...
    print(
        f"API Key: {ALPHA_VANTAGE_API_KEY[:10]}..."
        if ALPHA_VANTAGE_API_KEY
        else "❌ No API key configurada"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if time_series:
                ...
                print(
                    f"📊 Ejemplo - {first_date}: Close=${first_data.get('4. close', 'N/A')}"
                )
            else:
...
def test_news_api():
    ...
    params = {"q": "AAPL", "language": "en", "pageSize": 5, "apiKey": NEWS_API_KEY}
...
def check_config():
    ...
    print(
        f"ALPHA_VANTAGE_API_KEY: {'✅ Configurada' if ALPHA_VANTAGE_API_KEY else '❌ No configurada'}"
    )
