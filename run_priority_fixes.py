...
def main():
    ...
    if not run_command(
        "docker-compose -f docker-compose-simple.yaml down", "Deteniendo servicios"
    ):
        print("⚠️ Advertencia: Error al detener servicios")

    if not run_command(
        "docker-compose -f docker-compose-simple.yaml up -d", "Iniciando servicios"
    ):
    ...
    if not run_command(
        ...
        "Probando conexión desde backend",
    ):
    ...
    if not run_command(
        "docker exec proyectopuertagrande-backend-1 python /app/populate_real_data.py",
        "Poblando datos reales",
    ):
