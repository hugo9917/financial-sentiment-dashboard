...
all_news = []
print(
    f"[{datetime.now()}] Iniciando ingesta de noticias reales (NewsAPI) para {len(STOCK_SYMBOLS)} símbolos..."
)

for i, symbol in enumerate(STOCK_SYMBOLS):
    print(f"\n--- {symbol} ({i+1}/{len(STOCK_SYMBOLS)}) ---")
    try:
        news = fetch_historic_news(
            symbol, months=1
        )  # NewsAPI solo permite 30 días atrás
